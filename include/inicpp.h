...
class IniField
{
  ...
  public:
    ...
    template <typename T> IniField &operator=(const T &value)
    {
        Convert<T> conv;
...
template <typename Comparator> class IniSectionBase : public std::map<std::string, IniField, Comparator>
{
  public:
    IniSectionBase()
    {
...
template <typename Comparator> class IniFileBase : public std::map<std::string, IniSectionBase<Comparator>, Comparator>
{
  private:
    char fieldSep_ = '=';
    char esc_ = '\\';
    ...
    void eraseComment(const std::string &commentPrefix, std::string &str, std::string::size_type startpos = 0)
    {
  ...
  public:
    ...
    IniFileBase(std::istream &is, const char fieldSep, const std::vector<std::string> &commentPrefixes)
        : fieldSep_(fieldSep), commentPrefixes_(commentPrefixes)
    {
        decode(is);
    }
        ...
        while (!is.eof() && !is.fail())
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        std::stringstream ss;
                        ss << "l." << lineNo << ": ini parsing failed, duplicate field found";
                        throw std::logic_error(ss.str());
                    }
