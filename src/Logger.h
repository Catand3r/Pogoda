...
{
  private:
    Logger() = default; // nie by³ by to domyœlny konstruktor -> utworzyæ plik o konkretnej œcie¿ce
    ...
    void log(const std::string &message)
    {
        ...
        const std::string filename =
            ...
            (dt.day < 10 ? "0" : "") + std::to_string(dt.day) + "/" + std::to_string(dt.year) + "-" +
            (dt.month < 10 ? "0" : "") + std::to_string(dt.month) + "-" + (dt.day < 10 ? "0" : "") +
            std::to_string(dt.day) + "-" + (dt.hour < 10 ? "0" : "") + std::to_string(dt.hour) + ":" +
            (dt.minute < 10 ? "0" : "") + std::to_string(dt.minute) + ":" + (dt.second < 10 ? "0" : "") +
            std::to_string(dt.second) + "-" + std::to_string(dt.millisecond) + ".log";
